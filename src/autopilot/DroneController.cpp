...
ControlCommand DroneController::update(tum_ardrone::filter_stateConstPtr state)
{
	...
	if(targetValid)
		calcControl(new_err, d_err, yaw, -1);
	else
	{
...
ControlCommand DroneController::update(tum_ardrone::filter_stateConstPtr state, double maxControlFactor)
{
	TooN::Vector<3> pose = TooN::makeVector(state->x, state->y, state->z);
	double yaw = state->yaw;
	TooN::Vector<4> speeds = TooN::makeVector(state->dx, state->dy, state->dz, state->dyaw);
	...
	if(targetValid)
		calcControl(new_err, d_err, yaw, maxControlFactor);
	else
	{
		lastSentControl = hoverCommand;
		ROS_WARN("Warning: no valid target, sending hover.");
...
void DroneController::calcControl(TooN::Vector<4> new_err, TooN::Vector<4> d_error, double yaw, double maxControlFactor)
{
	...
	*/
    ...
    if (maxControlFactor > 0) {
        double mag = sqrt(pow(lastSentControl.roll, 2) + pow(lastSentControl.pitch, 2) + pow(lastSentControl.gaz, 2));
        if (mag > maxControlFactor) {    
            lastSentControl.roll *= maxControlFactor/mag;
            lastSentControl.pitch *= maxControlFactor/mag;
            lastSentControl.gaz *= maxControlFactor/mag;
        }
    }
